...
1. `codex-wechat notify-finish on|off|status` is the primary Desktop-side toggle. It is scoped to the current `CODEX_THREAD_ID`, so parallel Desktop threads can opt in independently.
2. `/notify on|off|status` is a WeChat-side convenience only when the current WeChat route is already attached to a Desktop thread.
...
5. If notifications are on for that Desktop thread, `send` stores a thread-scoped pending finish offer and proactively notifies WeChat.
6. Finish notifications should include a short `summary` and `next-action` so the user knows what finished and what decision is needed.
7. `/continue` consumes the latest pending finish offer:
...
8. Ignoring the notification changes no mobile session state. The previous WeChat project/session remains available.
9. If the Desktop user continued without first pulling mobile work, the notification should remind them that `pull WeChat back` is the exact path for importing phone-side raw transcript into Desktop.
...
3. `bun codex-wechat-ilink.ts notify-finish on --state-dir /tmp/codex-wechat-hook-smoke --to last --thread-id desktop-thread`
4. `bun codex-wechat-ilink.ts notify-finish send --state-dir /tmp/codex-wechat-hook-smoke --to last --project current --thread-id desktop-thread --summary "smoke done" --next-action "decide next step" --dry-run`
